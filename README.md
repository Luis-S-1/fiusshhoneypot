# FIU ssh honeypot

Project documentation

## Documentation
- [Introduction to git](docs/Intro_to_git.md)
- [Honeypot Installation](docs/FIU_Honeypot_HowToInstall.md)
- [Installing git on Windows](docs/Installing_git_on_windows.md)
- [Useful Github commands](docs/Introduction_to_Github.md)
- [Initial GitHub commit](docs/First_github_commit.md)
- [Adding Logback support to project](docs/Adding_Logback_support.md)

## License
License information goes here.
